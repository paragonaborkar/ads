...
public class OperationalOverride {
	...
	public void pageReach(WebDriver driver) {
		...
		new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(By.id("admin-op-override"))).click();
	}
	
	public void operationalOverride(WebDriver driver) {
		new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(By.id("btnReset"))).click();
		new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(By.id("requestedBy"))).sendKeys("SELENIUM-TEST");
