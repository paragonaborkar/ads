...
public class NFSDataImporterToDWH {
	...
	public static void main(String[] args) {
		...
		new NFSDataImporterToDWH().importData("", 1, "C:/netapp/host-connections/PowerShell To Java/NFSExports.json");
	}
	public void importData(String dataWarehouseHostName, int port, String filePath) {

		CombinedActiveExports combinedActiveExports = (CombinedActiveExports)jsonUtils.readFile(filePath, CombinedActiveExports.class);
		...
		if (combinedActiveExports != null) {
			...
			for (HostExportData exportEntry : exportList) {
				...
				//ExportsAndHost exportDataForCurrentHost=getExportDataForGivenQtreeOrVolume(dwhNfsExportList,qTreePath,volumePath);
				if(thisExportDetail!=null) {
					...
					nfsImportData.setHostId(hostId);
					...
					if(volumeName!=null|| volumeName.length()>0) {
						...
						if(storageObject!=null) {
							...
							System.out.println("Skipping entry: " + exportLocation + "is not in the DataWarehouse. We also failed to find the Volume (" + volumePath + ")" );
						}
					}
				}
			}
			...
			if(bulkImport.size()>0) {
				...
				for (NFSImportData nfsImportData : bulkImport) {
					...
					NaDBUtils.importNFSActiveHostInformation(nfsImportData.getShareId(),nfsImportData.getInternalVolId(),nfsImportData.getName(),nfsImportData.getProtocol(),nfsImportData.getHostId(),nfsImportData.getIp(),nfsImportData.getOperation());
				}
			}
			...
			if(activeHostsBulkImportLst.size()>0) {
				...
				for (ActiveHostBulkImport activeHostBulkImport : activeHostsBulkImportLst) {
					...
					Timestamp lastSeen=NaDBUtils.getCurrentTimeStamp();
					
					NaDBUtils.importInactiveHostInformation(strStorageId, protocol, hostId, hostIp, nfsOps);
...
///			#Completed		
			...
			/*for (HostExportData inActiveHostExportData: inActiveHosts) {
				...
				String protocol="NFS";
				String hostId=hostIpIdMap.get(inActiveHostExportData.getIp());
				String hostIp=inActiveHostExportData.getIp();
				//storageId;
				String nfsOps=inActiveHostExportData.getOperations();
				
				//insert into nfs_host_conn_nomount
				NaDBUtils.importInactiveHostInformation(storageId,protocol,hostId,hostIp,nfsOps);
			}*/
...
/*	public void importDataFromExportsAndHosts(ExportsAndHost exportDataForCurrentHost, HostExportData exportData, String hostId) {


		String shareId=exportDataForCurrentHost.getId();
		String internalVolId=exportDataForCurrentHost.getInternalVolumeId();
		String name=exportDataForCurrentHost.getName();
		String protocol=exportDataForCurrentHost.getProtocol();
		
		String ip=exportData.getIp();
		String operation=exportData.getOperations();
		

	}*/

	public ExportsAndHost getExportDataForGivenQtreeOrVolume(List<ExportsAndHost> lstDWHExportsAndHosts, String qTreePath, String volumePath) {
